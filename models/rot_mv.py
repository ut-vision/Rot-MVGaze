...
class IntensityBatchNorm(nn.Module):
    def __init__(
        self, n_channels: int, momentum: float = 0.05, eps: float = 1e-4
    ) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        intensity = torch.norm(
            x, dim=-2, keepdim=True
        ).detach()  # [batchsize, 1, feat_dim]
        ...
        if self.training:
            self.running_mean = (
                self.running_mean * (1 - self._momentum) + std * self._momentum
            )
...
class ImageFeatFuser(nn.Module):
    def __init__(self, img_feat_dim: int, num_feat_vec: int) -> None:
        ...
        self._fuser = Mlp(
            self.in_channel,
            [self.in_channel, num_feat_vec * 3],
...
class ImageRotmatFeatFuser(nn.Module):
    ...
    def forward(
        ...
        in_feat = torch.cat(
            [img_feat, rotatable_feat.flatten(-2, -1), rot.flatten(-2, -1)], dim=-1
        )
        out_feat = self._fuser(in_feat)
        return out_feat
...
class TransformerRotFuser(nn.Module):
    def __init__(self) -> None:
        self._fuser = nn.Transformer(
            d_model=3, nhead=1, num_decoder_layers=6, batch_first=True
        )
        ...
        if share_weights:
            ...
            if share_feature:
                ...
                self._gaze_estimators = nn.ModuleList(
                    [
                        Mlp(
                            self._num_feat_vec * 3 + self._fc_dim, out_channels=[512, 2]
                        )
                        for _ in range(num_iter)
                    ]
                )
    ...
    def forward(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for f_i, (img_fuser, gaze_estimator) in enumerate(
            ...
            if self._encode_rotmat:
                ...
                rotatable_feat_1 = img_fuser(img_feat_1, rotatable_feat_0_swap).reshape(
                    -1, 3, self._num_feat_vec
                )
            else:
                rotatable_feat_0 = img_fuser(
                    img_feat_0, rot_10 @ rotatable_feat_1
                ).reshape(-1, 3, self._num_feat_vec)
                rotatable_feat_1 = img_fuser(
