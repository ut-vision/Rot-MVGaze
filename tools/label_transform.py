...
from visualization import draw_gaze, plot_cameras
...
def mean_eye_nose(landmarks):
    ...
    face_center = torch.mean(
        torch.cat((two_eye_center, nose_center), dim=0), dim=0
    ).view(1, -1)
    return face_center
...
def pitchyaw_to_vector(pitchyaws):
    ...
    return torch.stack(
        [cos[:, 0] * sin[:, 1], sin[:, 0], cos[:, 0] * cos[:, 1]], 1
    ).float()
...
def vector_to_pitchyaw(vectors):
    ...
    return torch.stack(
        [torch.asin(vectors[:, 1]), torch.atan2(vectors[:, 0], vectors[:, 2])], dim=1
    ).float()  ## stack [ theta, phi]
...
def rotation_matrix_2d(pitch_yaw):
    ...
    """
    assert isinstance(
        pitch_yaw, torch.Tensor
    ), "make sure the pitchyaw here is torch.tensor"
...
def to_canonical(head, gaze):
    ...
    gaze_vector_canonical = torch.matmul(
        gaze_vector, rotation_mat.transpose(1, 2)
    ).squeeze(
        dim=1
    )  # (N, 1, 3)
...
def canonical_to_camera(head, gaze_canonical):
    ...
    rotation_mat = rotation_matrix_2d(pitch_yaw=head)  # (N, 3, 3) cononical --> head
    gaze_vector = torch.matmul(
        gaze_vector_canonical, rotation_mat.transpose(1, 2)
    ).squeeze(
        dim=1
    )  # (N, 1, 3)
...
def head_pose_to_camera_pose(hR, ht):
    ...
    cam_translation = -torch.matmul(
        ht.view(-1, 1, 3), cam_rotation.transpose(-2, -1)
    )  # the transpose here is because of the array multiplication
